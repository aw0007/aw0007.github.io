...
gg_niv_educ<-ggplot(data = dataCm, aes(x = fct_infreq(Niv_educ), 
  ...
  geom_text(aes(label = gsub("%", "", scales::percent(..count.. / sum(..count..)))),
            stat = "count", vjust = -0.7, size = 4, angle = 0, 
...
gg_statu_ocup<-ggplot(data = dataCm, aes(x = fct_infreq(statut_occupation), 
  ...
  theme_classic()+
  geom_text(aes(label = gsub("%", "", scales::percent(..count.. / sum(..count..)))),
            stat = "count", vjust = -0.5, size = 4, angle = 0, 
            position = position_dodge(width = 1))+
...
gg_Age_cm<-ggplot(data = dataCm, aes(x = fct_infreq(Age), 
  ...
  theme(legend.text = element_text(size = 8),
        ...
        axis.text.y = element_text(size = 10.5,angle = 0, hjust = 1)) +
  coord_flip()
...
gg_Mt_Eau_num_vs_Niv_educ <- ggplot(data = dataCm, aes(x = fct_infreq(Mt_Eau_num), 
  ...
  geom_bar(width = 0.96, position = position_dodge2()) +
  ...
  theme_classic() +
  geom_text(aes(label = gsub("%", "", scales::percent(..count.. / sum(..count..)))),
            stat = "count",hjust = 1, vjust = 0.4, size = 3,color = "#000000", angle = 90, 
            position = position_dodge(width = 1)) +
  theme(legend.text = element_text(size = 9),
        legend.key.size = unit(0.25, "cm"),
        axis.text.x = element_text(size = 10,angle = 25, hjust = 1),
...
gg_Mt_Eau_num_vs_Niv_educ_warp <-ggplot(data = dataCm, aes(x = fct_infreq(Mt_Eau_num), 
  ...
  scale_fill_manual(name = "Niveau d'éducation du CM",          labels = c("Neant", "Niveau Primaire", "Niveau Collegiale", "Niveau secondaire", "Niveau supérieur"), 
                    values = c("#2f4b7c", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd")) + 
  labs(title = " ",
       x = "", y = "") +
  guides(fill = guide_legend(title = NULL)) + 
  theme_classic() +
...
gg_Mt_Eau_num_vs_Revenu <- ggplot(data = dataCm, aes(x = fct_infreq(Mt_Eau_num), 
                                                       fill = factor(Revenu))) +
  geom_bar(width = 0.89, position = position_dodge2()) +
...
gg_Mt_Eau_num_vs_surface <- ggplot(data = dataCm, aes(x = fct_infreq(Mt_Eau_num), 
  ...
  theme_classic()+
  geom_text(aes(label = gsub("%", "", scales::percent(..count.. / sum(..count..)))),
            stat = "count", vjust = -0.5, size = 2.5, angle = 90, 
            position = position_dodge(width = 1))+
  theme(legend.text = element_text(size = 8),
        legend.key.size = unit(0.25, "cm"),
        axis.text.x = element_text(size = 10,angle = 25, hjust = 1),
...
gg_Mt_Eau_num_vs_taille_mn <- ggplot(data = dataCm, aes(x = fct_infreq(Mt_Eau_num), 
  ...
  theme_classic() +
  geom_text(aes(label = gsub("%", "", scales::percent(..count.. / sum(..count..)))),
            stat = "count", vjust = -0.5, size = 2.5, angle = 70, 
            position = position_dodge(width = 1))+
  theme(legend.text = element_text(size = 9),
        legend.key.size = unit(0.25, "cm"),
        axis.text.x = element_text(size = 10,angle = 25, hjust = 1),
...
liste_graphiques <- list(gg_taille_mn, 
                         gg_Age_cm,
                         gg_Revenu_cm,
                         gg_Type_logement,
                         gg_Mt_Eau_num,
...
noms_fichiers <- c("gg_taille_mn",
                   ...
                   "gg_Mt_Eau_num_vs_taille_mn",
                   "gg_Mt_Eau_num_vs_surface",
                   "gg_Mt_Eau_num_vs_Age_cm_mn",
                   "gg_Mt_Eau_num_vs_genre"
                   )


for (i in 1:length(liste_graphiques)) {
  nom_fichier <- paste0(noms_fichiers[i], ".pdf")
  ggsave(filename = nom_fichier, plot = liste_graphiques[[i]], dpi = 200)
}
